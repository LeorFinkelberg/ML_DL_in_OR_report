...
Лучший набор признаков, упорядоченный по убыванию пермутационной важности:
\begin{enumerate}
	\item \verb|var_lp_sol_default_presolve|,
	
	\item \verb|var_ub_global_relax|,
	
	\item \verb|relax_sol|,
	
	\item \verb|integer_context_types|,
	
	\item \verb|var_mean_relax_context|,
	
	\item \verb|binary_context_types|.
\end{enumerate}

Метрики качества (на валидационном поднаборе данных):
\begin{itemize}
	\item гармоическое среднее $ F_1 $: 0.718,
	\item полнота: 0.711,
	\item точность: 0.725,
	\item площадь под ROC-кривой: 0.720 (см. \pic{fig:roc_auc_zero_shot})
\end{itemize}

\begin{figure}[!h]
	\centering
	\includegraphics[scale=1.0]{figures/roc_auc_zero_shot_automl.png}
	\caption{ ROC-кривая задачи обнаружения <<квазинулевых>> переменных }\label{fig:roc_auc_zero_shot}
\end{figure}
