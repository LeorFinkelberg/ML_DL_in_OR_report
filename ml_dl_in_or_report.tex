...
\section{Концепт метрики сложности MILP-проблемы}

Метрику приделагается вычисляеть следующим образом

\begin{align*}
C &= \tau^{ (-1)^{ 1 - [\,\tau > 1\,] } } \cdot \dfrac{ \#ints_{\text{before}} }{\#zero\_ints_{\text{relax}}} \cdot \dfrac{ \#conss_{\text{after}} }{ \#conss_{\text{before}} } \cdot \dfrac{ \gamma_{\text{after}} }{ \gamma_{\text{before}} }, \\ \tau &= \dfrac{ \tau_{\text{relax}} }{ \tau_{\text{presolve}} },
\end{align*}
где

\noindent$ [\,\cdot\,] $ -- скобки Айверсона,
